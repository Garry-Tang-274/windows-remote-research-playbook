...
Remove only the scheduled task while keeping local files and GitHub history:

只移除定时任务，同时保留本地文件和 GitHub 历史：

```bash
crontab -l | grep -v '# tcr-sync-managed' | crontab -
```
